...
async def test_test_connection_success(hass: HomeAssistant) -> None:
    ...
    with patch("custom_components.wattbox.telnet_client.WattboxTelnetClient") as mock_client:
...
async def test_test_connection_auth_error(hass: HomeAssistant) -> None:
    ...
    with patch("custom_components.wattbox.telnet_client.WattboxTelnetClient") as mock_client:
        mock_instance = AsyncMock()
        mock_client.return_value = mock_instance
        mock_instance.async_connect.side_effect = WattboxAuthenticationError("Auth failed")
...
async def test_test_connection_connection_error(hass: HomeAssistant) -> None:
    ...
    with patch("custom_components.wattbox.telnet_client.WattboxTelnetClient") as mock_client:
        mock_instance = AsyncMock()
        mock_client.return_value = mock_instance
        mock_instance.async_connect.side_effect = WattboxConnectionError("Connection refused")
